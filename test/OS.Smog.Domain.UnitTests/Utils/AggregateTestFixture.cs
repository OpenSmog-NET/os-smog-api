...
namespace OS.Smog.Domain.UnitTests
{
        ...
        where TState : BaseState, new()
    {
        ...
        private static void Write(IEnumerable<IEvent> events)
        {
            foreach (var @event in events)
                Write(@event);
        }

        private static string DiscoverName(Expression action)
        {
        ...
        private static void Write(IEvent @event)
        {
            Console.WriteLine($"- {@event}");
        }

        // ReSharper disable once ReturnTypeCanBeEnumerable.Local
        private static byte[] Serialize(IEnumerable<IEvent> events)
        {
